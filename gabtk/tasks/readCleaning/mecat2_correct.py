...
class correctPAC(luigi.Task):
	projectName = GlobalParameter().projectName
	seq_platforms = luigi.Parameter(default="pac")
	...
	def run(self):
		mecat2_correct_folder = os.path.join(os.getcwd(), GlobalParameter().projectName,"ReadQC","CorrectReads","PAC-Reads" + "/")
		read_correction_log_folder = os.path.join(os.getcwd(), self.projectName,"log", "ReadQC", "CorrectReads", "PAC-Reads"+ "/")


		mecat2_assembly_folder=os.path.join(os.getcwd(), self.projectName,"GenomeAssembly", "MECAT" + "/")
		mecat2_corrected_fasta = os.path.join(os.getcwd(), self.projectName,"GenomeAssembly", "MECAT",self.assembly_name,"1-consensus","cns_final.fasta"+"/")
		...
		mecat_correct_config_path=os.path.join(os.getcwd(), "configuration", "mecat2_correct.config")
		...
		with open (mecat_correct_config_path,'w') as configfile:
		...
		mecat_cmd = "[ -d  {mecat2_assembly_folder} ] || mkdir -p {mecat2_assembly_folder}; " \
						"mkdir -p {read_correction_log_folder}; cd {mecat2_assembly_folder}; " \
						"/usr/bin/time -v mecat.pl correct {mecat_correct_config_path} " \
						"2>&1 | tee {read_correction_log_folder}mecat_assembly.log " \
			.format(mecat2_assembly_folder=mecat2_assembly_folder,mecat_correct_config_path=mecat_correct_config_path,
		...
		cp_corrected_fasta_cmd="[ -d  {mecat2_correct_folder} ] || mkdir -p {mecat2_correct_folder}; " \
							   "cp {mecat2_corrected_fasta} {mecat2_correct_folder}".format(mecat2_correct_folder=mecat2_correct_folder,mecat2_corrected_fasta=mecat2_corrected_fasta)
