...
class falcon(luigi.Task):
	...
	min_contig_length = luigi.Parameter(description="Minimum contig length")
	...
	def run(self):
		...
		falcon_assembly_folder = os.path.join(os.getcwd(), GlobalParameter().projectName, "GenomeAssembly", "FALCON_" + self.seq_platform,self.assembly_name +"/")
		falcon_assembly_log_folder=s.path.join(os.getcwd(), GlobalParameter().projectName, "log","GenomeAssembly", "FALCON_" + self.seq_platform,self.assembly_name +"/")
		...
		def fofn(lrfile,inputDir):
			...
			clean_read_list= os.path.join(os.getcwd(), "CleanedReads", "Cleaned_Long_Reads", "input.fofn")
		...
		if self.pre_process_reads=="yes" and self.seq_platform=="pacbio":
			fastq_input = falcon_fastq(pac_sample_list, cleaned_pac_read_folder)
			fasta_out=falcon_fasta(pac_sample_list,cleaned_pac_read_folder)
			fasta_rename=falcon_fasta_rename(pac_sample_list,cleaned_pac_read_folder)
			

		if self.pre_process_reads=="yes" and self.seq_platform=="nanopore":
			fastq_input = falcon_fastq(ont_sample_list, cleaned_ont_read_folder)
			fasta_out=falcon_fasta(ont_sample_list,cleaned_ont_read_folder)
			fasta_rename=falcon_fasta_rename(ont_sample_list,cleaned_ont_read_folder)

		if self.pre_process_reads=="no" and self.seq_platform=="pacbio":
			fastq_input = falcon_fastq(pac_sample_list, verified_pac_read_folder)
			fasta_out=falcon_fasta(pac_sample_list,verified_pac_read_folder)
			fasta_rename=falcon_fasta_rename(pac_sample_list,verified_pac_read_folder)
			

		if self.pre_process_reads=="no" and self.seq_platform=="nanopore":
			fastq_input = falcon_fastq(ont_sample_list, verified_ont_read_folder)
			fasta_out=falcon_fasta(ont_sample_list,verified_ont_read_folder)
			fasta_rename=falcon_fasta_rename(ont_sample_list,verified_ont_read_folder)
		...
		.format(lr_cleaned_read_folder=lr_cleaned_read_folder,fastq_input=fastq_input,
					fasta_out=fasta_out)
